...
where (node = API::moduleImport("django").getMember("contrib").getMember("auth").getMember("decorators").getMember("login_required").getAValueReachableFromSource()
        or node = API::moduleImport("django").getMember("contrib").getMember("auth").getMember("mixins").getMember("LoginRequiredMixin").getAValueReachableFromSource())
    ...
    and if exists(getArgumentFunction(node.asExpr())) or exists(getArgumentClass(node.asExpr())) or exists(getDecoratedFunction(node.asCfgNode())) or exists(getDecoratedClass(node.asCfgNode()))
        ...
        else if DjangoSession::getClassViews().getABase() = node.asExpr()
            then exists(Class cls |
                cls = DjangoSession::getClassViews()
                and cls.getABase() = node.asExpr()
                and name = cls.getName() + " | " + cls.getLocation())
            else name = "No class or view found"
select node, node.getLocation(), name, "The application is enabling csrf protection for certain views"
