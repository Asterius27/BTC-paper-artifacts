...
where config.hasFlow(source, sink)
    and (source.asExpr().(List).getAnElt().(StrConst).getText() = "django_registration"
        or source.asExpr().(Tuple).getAnElt().(StrConst).getText() = "django_registration")
select source, source.getLocation(), "django-registration (ubernostrum) is being used by the application"
