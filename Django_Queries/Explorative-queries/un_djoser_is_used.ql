...
where config.hasFlow(source, sink)
    and (source.asExpr().(List).getAnElt().(StrConst).getText() = "djoser"
        or source.asExpr().(Tuple).getAnElt().(StrConst).getText() = "djoser")
select source, source.getLocation(), "djoser is being used by the application"
