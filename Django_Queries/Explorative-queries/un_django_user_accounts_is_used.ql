...
where config.hasFlow(source, sink)
    and (source.asExpr().(List).getAnElt().(StrConst).getText() = "account"
        or source.asExpr().(Tuple).getAnElt().(StrConst).getText() = "account")
select source, source.getLocation(), "django-user-accounts is being used by the application"
