...
where config.hasFlow(source, sink)
    and (source.asExpr().(List).getAnElt().(StrConst).getText() = "knox"
        or source.asExpr().(Tuple).getAnElt().(StrConst).getText() = "knox")
select source, source.getLocation(), "knox is being used by the application"
