...
where config.hasFlow(source, sink)
    and (source.asExpr().(List).getAnElt().(StrConst).getText() = "allauth"
        or source.asExpr().(Tuple).getAnElt().(StrConst).getText() = "allauth"
        or source.asExpr().(List).getAnElt().(StrConst).getText().prefix(8) = "allauth."
        or source.asExpr().(Tuple).getAnElt().(StrConst).getText().prefix(8) = "allauth.")
select source, source.getLocation(), "Django-allauth is being used by the application"
