...
where config.hasFlow(source, sink)
    and (source.asExpr().(List).getAnElt().(StrConst).getText() = "dj_rest_auth"
        or source.asExpr().(Tuple).getAnElt().(StrConst).getText() = "dj_rest_auth")
select source, source.getLocation(), "dj-rest-auth is being used by the application"
