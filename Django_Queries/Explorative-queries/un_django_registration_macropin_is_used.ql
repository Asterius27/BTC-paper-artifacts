...
where config.hasFlow(source, sink)
    and (source.asExpr().(List).getAnElt().(StrConst).getText() = "registration"
        or source.asExpr().(Tuple).getAnElt().(StrConst).getText() = "registration")
select source, source.getLocation(), "django-registration (macropin) is being used by the application"
