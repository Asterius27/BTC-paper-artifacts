...
where config.hasFlow(source, sink)
    ...
    and (pair.getValue().(StrConst).getS() = "django.contrib.auth.password_validation.NumericPasswordValidator"
        or pair.getValue().(BinaryExpr).getLeft().(StrConst).getS() + pair.getValue().(BinaryExpr).getRight().(StrConst).getS() = "django.contrib.auth.password_validation.NumericPasswordValidator")
select pair.getLocation(), source, sink, source.getLocation(), sink.getLocation(), "Using a numeric password validator (checking that the password is not entirely numeric)"
