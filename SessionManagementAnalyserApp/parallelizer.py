...
parser.add_argument("-su", dest="starsu", help="Upper bound on the number of stars for each repository", default=1000000000000, type=int) # TODO star filtering is disabled for now
parser.add_argument("-sl", dest="starsl", help="Lower bound on the number of stars for each repository", default=0, type=int) # TODO star filtering is disabled for now
...
if __name__ == '__main__':
    start = time.time()
    # codeql_threads = 1
    codeql_threads = (cpu_count() // args.threads) + 10
    j = 0
    current_thread = 0
