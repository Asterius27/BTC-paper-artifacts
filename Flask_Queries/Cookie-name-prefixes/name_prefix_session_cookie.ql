...
where not exists(Expr expr | 
  (expr = getConfigValueFromObject("SESSION_COOKIE_NAME")
    or expr = getConfigValueFromPyFile("SESSION_COOKIE_NAME")
    or expr = getConfigValue("SESSION_COOKIE_NAME")
    or expr = getConfigValueFromDictionary("SESSION_COOKIE_NAME"))
  and valueCheck(expr))
select "Session cookie doesn't use either the __Host- or __Secure- prefixes"

/*
from Expr expr
where (expr = getConfigValueFromObject("SESSION_COOKIE_NAME")
    or expr = getConfigValueFromPyFile("SESSION_COOKIE_NAME")
    or expr = getConfigValue("SESSION_COOKIE_NAME")
    or expr = getConfigValueFromDictionary("SESSION_COOKIE_NAME"))
  and valueCheck(expr)
...
where not exists(DataFlow::Node node, KeyValuePair kv | 
    ...
    or node = Flask::FlaskApp::instance().getMember("config").getMember("update").getKeywordParameter("SESSION_COOKIE_NAME").getAValueReachingSink())
