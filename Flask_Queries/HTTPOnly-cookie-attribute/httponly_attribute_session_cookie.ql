...
from Expr expr
where (expr = getConfigValueFromObject("SESSION_COOKIE_HTTPONLY")
    or expr = getConfigValueFromPyFile("SESSION_COOKIE_HTTPONLY")
    or expr = getConfigValue("SESSION_COOKIE_HTTPONLY")
    or expr = getConfigValueFromDictionary("SESSION_COOKIE_HTTPONLY"))
  and valueCheck(expr)
select expr, expr.getLocation(), "Session cookie is accessible via javascript (HTTPOnly attribute set to false)"
