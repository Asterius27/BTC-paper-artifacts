...
from Expr expr
where (expr = getConfigValueFromObject("REMEMBER_COOKIE_HTTPONLY")
    or expr = getConfigValueFromPyFile("REMEMBER_COOKIE_HTTPONLY")
    or expr = getConfigValue("REMEMBER_COOKIE_HTTPONLY")
    or expr = getConfigValueFromDictionary("REMEMBER_COOKIE_HTTPONLY"))
  and valueCheck(expr)
select expr, expr.getLocation(), "Remember cookie is accessible via javascript (HTTPOnly attribute set to false)"
...
where ((node = Flask::FlaskApp::instance().getMember("config").getSubscript("REMEMBER_COOKIE_HTTPONLY").getAValueReachingSink()
    or node = Flask::FlaskApp::instance().getMember("config").getMember("update").getKeywordParameter("REMEMBER_COOKIE_HTTPONLY").getAValueReachingSink())
    ...
    and kv = node.asExpr().(Dict).getAnItem()
    and kv.getKey().(Str).getText() = "REMEMBER_COOKIE_HTTPONLY"
