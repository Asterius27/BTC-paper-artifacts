...
from Expr expr
where (expr = getConfigValueFromObject("REMEMBER_COOKIE_DOMAIN")
    or expr = getConfigValueFromPyFile("REMEMBER_COOKIE_DOMAIN")
    or expr = getConfigValue("REMEMBER_COOKIE_DOMAIN")
    or expr = getConfigValueFromDictionary("REMEMBER_COOKIE_DOMAIN"))
  and valueCheck(expr)
select expr, expr.getLocation(), expr.(StrConst).getText(), "Remember cookie domain attribute is set"
...
where ((node = Flask::FlaskApp::instance().getMember("config").getSubscript("REMEMBER_COOKIE_DOMAIN").getAValueReachingSink()
    or node = Flask::FlaskApp::instance().getMember("config").getMember("update").getKeywordParameter("REMEMBER_COOKIE_DOMAIN").getAValueReachingSink())
    ...
    and kv = node.asExpr().(Dict).getAnItem()
    and kv.getKey().(Str).getText() = "REMEMBER_COOKIE_DOMAIN"
