...
where (node = Flask::FlaskApp::instance().getMember("config").getSubscript("REMEMBER_COOKIE_SAMESITE").getAValueReachingSink()
    and node.asExpr().toString() = "None")
    or not exists(Flask::FlaskApp::instance().getMember("config").getSubscript("REMEMBER_COOKIE_SAMESITE").getAValueReachingSink())
