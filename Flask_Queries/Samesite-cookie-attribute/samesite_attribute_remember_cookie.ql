...
where not exists(Expr expr | 
  (expr = getConfigValueFromObject("REMEMBER_COOKIE_SAMESITE")
    or expr = getConfigValueFromPyFile("REMEMBER_COOKIE_SAMESITE")
    or expr = getConfigValue("REMEMBER_COOKIE_SAMESITE")
    or expr = getConfigValueFromDictionary("REMEMBER_COOKIE_SAMESITE"))
  and valueCheck(expr))
...
where not exists(DataFlow::Node node, KeyValuePair kv | 
    ((node = Flask::FlaskApp::instance().getMember("config").getSubscript("REMEMBER_COOKIE_SAMESITE").getAValueReachingSink()
    or node = Flask::FlaskApp::instance().getMember("config").getMember("update").getKeywordParameter("REMEMBER_COOKIE_SAMESITE").getAValueReachingSink())
