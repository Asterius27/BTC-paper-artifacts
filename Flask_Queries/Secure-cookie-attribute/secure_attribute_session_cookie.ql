...
where not exists(Expr expr | 
  (expr = getConfigValueFromObject("SESSION_COOKIE_SECURE")
    or expr = getConfigValueFromPyFile("SESSION_COOKIE_SECURE")
    or expr = getConfigValue("SESSION_COOKIE_SECURE")
    or expr = getConfigValueFromDictionary("SESSION_COOKIE_SECURE"))
  and valueCheck(expr))
...
where not exists(DataFlow::Node node, KeyValuePair kv | 
    ((node = Flask::FlaskApp::instance().getMember("config").getSubscript("SESSION_COOKIE_SECURE").getAValueReachingSink()
    or node = Flask::FlaskApp::instance().getMember("config").getMember("update").getKeywordParameter("SESSION_COOKIE_SECURE").getAValueReachingSink())
    ...
    and kv = node.asExpr().(Dict).getAnItem()
    and kv.getKey().(Str).getText() = "SESSION_COOKIE_SECURE"
